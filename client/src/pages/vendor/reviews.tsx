  ...
  return (
      ...
      <div className="space-y-4 sm:space-y-6 relative z-10 p-3 sm:p-0">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
          ...
          <Card className="border border-gray-700 bg-gray-900">
            <CardContent className="p-4 sm:p-6">
              <div className="text-xl sm:text-2xl font-bold text-green-600">{ratingCounts[4]}</div>
                ...
                <div key={review.id} className="border-b border-gray-700 bg-gray-900 pb-4 sm:pb-6 last:border-b-0 last:pb-0">
                    ...
                    <div className="flex items-center space-x-3 sm:space-x-4 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center space-x-2 flex-wrap">
                          ...
                          {review.verified && (
                            <Badge variant="outline" className="text-[10px] sm:text-xs bg-green-50 text-green-700 border-green-200 flex-shrink-0">
                              Verified
                            </Badge>
                          )}
                        </div>
                  ...
                  {(review.reply || (review.conversation && review.conversation.length > 0)) && (
                    <div className="mt-3 sm:mt-4 ml-0 sm:ml-14 space-y-2">
                      ...
                      {review.reply && (!review.conversation || review.conversation.length === 0) && (
                        <div className="p-3 sm:p-4 bg-blue-900/30 rounded-lg border-l-4 border-blue-500">
                          <div className="flex items-center space-x-2 mb-2 flex-wrap">
                            <MessageCircle className="w-3 h-3 sm:w-4 sm:h-4 text-blue-400" />
                            <span className="font-medium text-blue-300 text-xs sm:text-sm">Your Reply</span>
                            <span className="text-[10px] sm:text-xs text-blue-400/70">{new Date(review.reply.date).toLocaleString()}</span>
                          </div>
                          <p className="text-blue-200 text-xs sm:text-sm break-words">{review.reply.content}</p>
                        </div>
                      )}
                      ...
                      {review.conversation && review.conversation.length > 0 && review.conversation.map((msg: any, idx: number) => (
                        ...
                        >
                          <div className="flex items-center space-x-2 mb-2 flex-wrap">
                            <MessageCircle className={`w-3 h-3 sm:w-4 sm:h-4 ${msg.author === 'vendor' ? 'text-blue-400' : 'text-green-400'}`} />
                            <span className={`font-medium text-xs sm:text-sm ${msg.author === 'vendor' ? 'text-blue-300' : 'text-green-300'}`}>
                              {msg.author === 'vendor' ? 'You' : review.buyer}
                            </span>
                            {msg.date && (
                              <span className={`text-[10px] sm:text-xs ${msg.author === 'vendor' ? 'text-blue-400/70' : 'text-green-400/70'}`}>
                                {new Date(msg.date).toLocaleString()}
                              </span>
                            )}
                          </div>
                          <p className={`text-xs sm:text-sm break-words ${msg.author === 'vendor' ? 'text-blue-200' : 'text-green-200'}`}>
                            {msg.message}
                          </p>
                        </div>
                      ))}
