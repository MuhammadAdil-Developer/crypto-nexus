...
class OrderService {
  ...
  async getAdminDashboard(days: number = 30, refresh: boolean = false): Promise<any> {
    try {
      const response = await api.get(`/orders/admin_dashboard/?days=${days}${refresh ? '&refresh=true' : ''}`);
      return response.data;
    } catch (error: any) {
